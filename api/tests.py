...
class ContactTestCase(APITestCase):
    ...
    def test_create_contact_when_email_equal_blank(self):
        """
    ...
    def test_create_contact_when_email_equals_non_email(self):
        """
        test ContactViewSet create method when email is not an email
        """
        data = self.data
        data['email'] = "test"
        response = self.client.post(self.url, data)
...
class EcommerceTestCase(APITestCase):
    ...
    def test_create_order_with_more_than_stock(self):
        """
        test OrdersViewSet create method when order.quantity > item.
        """
        for i in self.items:
            stock = i.stock
